...
export function Checkbox({ name, text, onChange, checked, disabled }: Checkbox) {
    return (
        <>
            <div className="cb-custom-cls">
                <label>
                    <input name={name} id={name} type="checkbox" checked={checked} onChange={onChange} disabled={disabled} />
                    <div tabIndex={0} onKeyDown={(event) => event.key === "Enter" && onChange()} />
                    <p>{text}</p>
                </label>
            </div>
            <style jsx>{`
