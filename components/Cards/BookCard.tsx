    ...
    useEffect(() => {
        if (typeof window !== undefined) {
            if (localStorage.getItem("state") != null || localStorage.getItem("state") != undefined) {
                ...
                const itemExists: Product | undefined = cart.find((item: Product) => item.id == product.id);
                ...
                <div className="p-5">
                    ...
                    {product.stock < 4 && product.stock > 1 && !product.digital && (
                        <p className="italic text-red-500 font-bold">
                            {langEn ? `Last ${product.stock} products` : `Ultimele ${product.stock} produse`}
                        </p>
                    )}
                    {product.stock === 1 && !product.digital && (
                        <p className="italic text-red-500 font-bold">{langEn ? `Last product` : `Ultimul produs`}</p>
                    )}
                    {product.stock === 0 && !product.digital && (
                        <p className="italic text-red-500 font-bold">{langEn ? `Out of stock` : `Nu mai este în stoc`}</p>
                    )}
