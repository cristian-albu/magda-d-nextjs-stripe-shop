...
const ContactPage = () => {
    ...
    const emailInvalidCheck: FormValidationFunc = () => {
        ...
        } else if (!/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(emailState.value)) {
            return {
    ...
    const messageInvalidCheck: FormValidationFunc = () => {
        if (messageState.value.length < 10) {
            return {
                message: langEn ? "Your message is too short" : "Mesajul ar trebui să fie puțin mai lung",
                err: true,
            };
        } else if (messageState.value.length > 500) {
            return {
                message: langEn ? "Your message is too long" : "Mesajul tău este prea lung",
                err: true,
            };
        } else {
            return { message: "", err: false };
    ...
    const { setCookiePref, setCookieAnalytics, openPrivacy, langEn, handleSave }: any = useContext(PrivacyContext);
    ...
    async function handleSubmit(e: any) {
        ...
        if (!nameInvalidCheck().err && !emailInvalidCheck().err && !messageInvalidCheck().err && checkboxState) {
                        ...
                        <div className="flex w-full items-center justify-between relative flex-wrap">
                            <div className="w-full md:w-1/2  flex flex-col text-white md:pr-[5rem] items-start">
                                ...
                                <h1 className="text-2xl md:text-5xl mb-5 ">
                                    {langEn
                                        ? "Send me a message, a review of the book or find me here:"
                                        : `Trimite un mesaj, o recenzie a cărții sau găseşte-mă aici:`}
                                </h1>
                                <div className="flex gap-5 flex-wrap">
                                    <a
                                        href="https://www.facebook.com/magdadimitrescu.oficial"
                                    ...
                                    <a
                                        href="https://www.instagram.com/magda.dimitrescu"
                                        rel="noopener nereferrer"
                                        target="_blank"
                                        className="btnPrimary mb-5"
                                    >
                                    ...
                                    >
                                        <div className="w-full h-full flex flex-col p-[1rem] md:p-[2rem] bg-white rounded-md">
                                            ...
                                            <div className="flex flex-wrap items-center justify-between">
                                                ...
                                                <button className="btnPrimary2 ml-auto" type="submit" name="submit">
                                                    <BiMessageDetail />
                                                    {langEn ? "Send" : "Trimite"}
                                                </button>
                                            </div>
                            ...
                            {mailSentSuccess && (
                                <div className="bg-white p-[2rem] shadow-black/20 shadow-2xl rounded-md w-full  md:w-1/2 mb-[3rem] flex justify-center items-center flex-col">
                                    ...
                                    <Link href={"/"} className="btnPrimary2">
                                        {langEn ? "Go to the home page" : "Mergi înapoi pe prima pagină"}
                                    </Link>
                                </div>
                            )}
                        </div>
